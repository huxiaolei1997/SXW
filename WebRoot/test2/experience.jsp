...
<script type="text/javascript">
	...
	function handlePaginationClick(new_page_index, pagination_container) {
		...
		$("#pager").attr("action", "<%=request.getContextPath()%>/test2/pagerExperience?pageNum=" + (new_page_index + 1));
		$("#pager").submit();
		return false;
	}
...
<li class="tpl-left-nav-item"><a href="<%=path%>/test2/showAccessIpAddress" class="nav-link"> <i class="am-icon-bar-chart"></i> <span>网站访客统计</span>
			...
			<div class="tpl-portlet-components">
				<div class="portlet-title">
					<div class="caption font-green bold">
						<span class="am-icon-code"></span> 体验管理&nbsp;&nbsp;
				...
				<div class="tpl-block">
							...
							<div class="am-btn-toolbar">
								<div class="am-btn-group am-btn-group-xs">
									<button type="button"
										...
										onclick="javascript:window.location.href='<%=path%>/test2/addExperience.jsp'">
										<span class="am-icon-plus"></span> 新增
									</button>
								</div>
							</div>
						</div>
						<form action="<%=path%>/test2/pagerExperience" id="pager"
					...
					<c:if test="${keyword != '' && keyword != null }">
						<c:if test="${fn:length(result) ne 0 }">
							<span>在<c:if test="${type == 'shortterm' }">
									<span style="color:red;">短期活动</span>中查询
												<span style="color:red">${keyword }</span>
								</c:if> <c:if test="${type == 'longterm' }">
									<span style="color:red;">长期活动</span>中查询
												<span style="color:red">${keyword }</span>
					...
					<div class="am-g">
						<div class="tpl-table-images">
							<c:forEach items="${result}" var="experience">
								<div class="am-u-sm-12 am-u-md-6 am-u-lg-4">

									<div class="tpl-table-images-content">
										<div class="tpl-table-images-content-i-time">发布时间：${fn:substring(experience.addtime,0,10) }
										<c:if test="${experience.display == '1'}"><span class="am-icon-eye am-icon-sm" style="float:right;"></span></c:if>
										<c:if test="${experience.display == '0'}"><span class="am-icon-eye-slash am-icon-sm" style="float:right;"></span></c:if></div>
										<div class="tpl-i-title">${experience.experTitle}</div>
										<a href="javascript:;" class="tpl-table-images-content-i">
											<div class="tpl-table-images-content-i-info">
												<span class="ico"> <img src="<%=path%>${experience.imageUrl}"
													alt=""><c:if test="${experience.type == 'shortterm'}">
													短期活动
													</c:if>
													<c:if test="${experience.type == 'longterm'}">
													长期活动
													</c:if>
												</span>

											</div> <span class="tpl-table-images-content-i-shadow"></span> <img
											src="<%=path%>${experience.imageUrl}" alt=""
											style="width:100%;height:150px;">
										</a>
										<div class="tpl-table-images-content-block">
											<div class="tpl-i-font">${experience.content }</div>
											<div class="tpl-i-more">
												<ul>
													<li class="am-text-warning"><span class="am-icon-thumbs-up am-text-warning am-icon-sm"></span> ${experience.likes}</li>
													<li class="am-text-success"><span class="am-icon-rss-square am-text-success am-icon-sm"></span> ${experience.browsingTimes}</li>
											...
											<div class="am-btn-toolbar">
												<div
													...
													<button type="button"
														...
														onclick="javascript:window.location.href='<%=path%>/test2/updateExperience?id=${experience.id }'">
														<span class="am-icon-edit"></span> 编辑
													</button>
													<button type="button"
														class="am-btn am-btn-default am-btn-warning" data-am-modal="{target: '#verify', closeViaDimmer: 0, width: 400, height: 130}" onclick="show('${experience.display}','${experience.id}')">
														<span class="am-icon-archive"></span> 审核
													</button>							
													<button type="button"
														class="am-btn am-btn-default am-btn-danger"
														onclick="deleteExperience('${experience.id}','${experience.imageUrl}')">
  ...
  <div class="am-modal-dialog">
	...
	<script type="text/javascript">
		...
		function deleteExperience(id,imageUrl) {
			swal({
				title : '确定删除吗？',
				text : '你将无法恢复它！',
				type : 'warning',
				...
				initAJAX();
				xmlHttp.open("POST", '<%=path%>/test2/deleteExperience?id=' + id + "&imageUrl=" + imageUrl, true);
				xmlHttp.send();
	...
	//alert(showViewValue);
	initAJAX();
	xmlHttp.open("POST","<%=path%>/test2/showOrNotShow?display=" + showViewValue + "&updateId=" + updateId.value + "&updateType=experience",true);
	xmlHttp.send();
	xmlHttp.onreadystatechange = function(){
	if(xmlHttp.readyState == 4 && xmlHttp.status == 200){
	var res = xmlHttp.responseText;
