...
<li class="tpl-left-nav-item"><a href="<%=path%>/test2/showAccessIpAddress" class="nav-link"> <i class="am-icon-bar-chart"></i> <span>网站访客统计</span>
					...
					<div class="am-g">
						<div class="tpl-form-body tpl-form-line">
							<form class="am-form tpl-form-line-form"
								action="<%=path%>/test2/addXiangJu" method="post"
								...
								<div class="am-form-group">
									...
									<div class="am-u-sm-9">
										<input type="text" class="tpl-form-input" id="roomTitle"
											placeholder="请输入民宿类型" name="roomTitle" maxlength="20">
										<small>请填写民宿类型最多20字。</small>
									</div>
								</div>
								<div class="am-form-group">
									...
									<div class="am-u-sm-9">
										<textarea id="feature" rows="5" placeholder="请输入民宿介绍文字"
											name="feature" maxlength="100"></textarea>
										<small>请填写民宿介绍文字最多100字。</small>
									</div>
								</div>
								<div class="am-form-group">
	...
	<script type="text/javascript">
		function uploadImage(files) {
			...
			var uploadimage = document.forms.namedItem("uploadimage");
			oData = new FormData(uploadimage);
			oData.append("CustomField", "This is some extra data");
			//var oReq = new XMLHttpRequest();
			xmlHttp.open("POST", "<%=path%>/test2/upload", true);
			xmlHttp.onload = function(oEvent) {
				if (xmlHttp.status == 200) {
					//var res = "/sxw" + xmlHttp.responseText + "?t=" + Math.random();
					//var  res = "/sxw" + xmlHttp.responseText + "?t=" + Math.random();
					...
					if (res.indexOf(";") >= 0) {
						...
						for (var i = 0; i < strs.length - 1; i++) {
							showimage[i].src = "<%=path%>" + strs[i] + "?t=" + Math.random();
							imageUrl.value = xmlHttp.responseText;
						//alert(strs[i]);
						}
					} else {
						for (var i = 0; i < 1; i++) {
							showimage[i].src = "<%=path%>" + res + "?t=" + Math.random();
							imageUrl.value = xmlHttp.responseText;
						// alert(res);
						}
						for (var i = 1; i <= 4; i++) {
							showimage[i].src = "" + "?t=" + Math.random();
						}
					}
