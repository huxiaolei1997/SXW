...
<script type="text/javascript">
	...
	function handlePaginationClick(new_page_index, pagination_container) {
		...
		$("#orderXiangJu").attr("action", "<%=request.getContextPath()%>/test2/showBackIndex?pageNum=" + (new_page_index + 1));
		$("#orderXiangJu").submit();
		return false;
	}
				...
				<ul
					...
					<li><a href="<%=path%>/test2/showBackIndex"
						class="am-text-success"><span
							class="am-icon-btn am-icon-file-text"></span><br />乡居订单管理<br />${totalPages}</a></li>
					<li><a href="javascript:;" class="am-text-danger" style="cursor:default;"><span
							class="am-icon-btn am-icon-recycle"></span><br />未审核订单<br />${totalPages - countReviewXiangJu }</a></li>
					<li><a href="javascript:;" class="am-text-secondary" style="cursor:default;"><span
							class="am-icon-btn am-icon-user-md"></span><br />已审核订单<br />${countReviewXiangJu}</a></li>
				</ul>
				...
				<div class="am-g">
					<div class="am-u-sm-12">
						<table
							...
							<tbody>
								<c:forEach items="${result}" var="orderXiangJu">
									<tr>
										<td><c:if test="${orderXiangJu.review == '1' }"><p style="display:inline-block;color:red;">（已审核）</p></c:if><c:if test="${orderXiangJu.review == '0' }"><p style="display:inline-block;">（未审核）</p></c:if>${orderXiangJu.id}</td>
										<td>${orderXiangJu.userName}</td>
										<td>${orderXiangJu.realName}</td>
										<td>${orderXiangJu.phoneNumber}</td>
										<td>${orderXiangJu.houseType}</td>
										<td>${orderXiangJu.checkInDate}至
											${orderXiangJu.checkOutDate}</td>
										<td>${orderXiangJu.checkInNumber}</td>
										<td><c:if
												test="${orderXiangJu.requirement == null || orderXiangJu.requirement == '' }">无</c:if>
											<c:if
												test="${orderXiangJu.requirement != '' || orderXiangJu.requirement != null}">${orderXiangJu.requirement}</c:if></td>
										<td>
											<div class="am-dropdown" data-am-dropdown>
												...
												<ul class="am-dropdown-content">
													<li><a href="javascript:;"
														onclick="update('${orderXiangJu.id}','${orderXiangJu.checkInNumber}','${orderXiangJu.checkInDate}','${orderXiangJu.checkOutDate}','${orderXiangJu.requirement}','${orderXiangJu.houseType}')"
														class="am-btn"
														data-am-modal="{target: '#doc-modal-3', closeViaDimmer: 0, width: 400, height: 390}"><c:if test="${orderXiangJu.review == '1'}">重新审核</c:if><c:if test="${orderXiangJu.review == '0'}">审核</c:if></a></li>
													<li><a href="javascript:;"
														onclick="deleteOrderXiangJu('${orderXiangJu.id}','orderXiangJu')">删除</a></li>
												</ul>
											</div>
										</td>
									</tr>
						...
						<form action="<%=path%>/test2/showBackIndex" method="post"
							id="orderXiangJu"></form>
					</div>
				</div>
...
<!-- 
	...
	<div class="am-modal am-modal-no-btn" tabindex="-1" id="doc-modal-3">
		<div class="am-modal-dialog">
			...
			<div class="am-modal-bd">
				<form action="<%=path%>/test2/updateOrderXiangJu" method="post"
					id="updateXiangJu">
					<input type="hidden" id="updateId" name="updateId" /> <input
						type="text" name="checkInDate" class="am-form-field"
						placeholder="选择入住日期" data-am-datepicker readonly /><br> <input
						type="text" name="checkOutDate" class="am-form-field"
						placeholder="选择离开日期" data-am-datepicker readonly /><br> <input
						type="text" name="checkInNumber" class="am-form-field am-radius"
						placeholder="预定人数" maxlength="3" /><br> <input type="text"
						name="requirement" class="am-form-field am-radius"
						placeholder="特殊要求，可以不填" /><br> <label style="float:left;">有效订单：<input
					...
					<label style="float:left;display:none;" id="reason">不通过理由：<input
						...
						style="border:1px solid red;" maxlength="100" /></label><br /> <label
						style="float:left;"><input type="button" value="确定"
						class="am-btn am-btn-success" onclick="updateOrderXiangJu()" /></label>
				</form>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function update(id, checkInNumber1, checkInDate1, checkOutDate1, requirement1, houseType) {
			var checkInDate = document.getElementsByName("checkInDate")[0];
			var checkOutDate = document.getElementsByName("checkOutDate")[0];
			var checkInNumber = document.getElementsByName("checkInNumber")[0];
			...
			checkInNumber.value = checkInNumber1;
			requirement.value = requirement1;
			updateId.value = id;
	
		}
		function updateOrderXiangJu() {
			...
			for (var i = 0; i < radioPass.length; i++) {
				if (radioPass[i].checked) {
					if (radioPass[i].value == '1') {
						if (checkInDate.value != '' && checkOutDate.value != '' && checkInNumber.value != '')
							updateXiangJu.submit();
					}
	
					if (radioPass[i].value == '0') {
						if (checkInDate.value != '' && checkOutDate.value != '' && checkInNumber.value != '' && passOrNotPass.value != '')
							updateXiangJu.submit();
					}
				}
			}
		}
		function deleteOrderXiangJu(id, orderXiangJu) {
			...
			swal({
				...
				initAJAX();
				xmlHttp.open("POST", "<%=path%>/test2/deleteOrderXiangJu?id=" + id + "&order=" + orderXiangJu, true);
				xmlHttp.send();
