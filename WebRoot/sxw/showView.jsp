...
%>
<c:forEach items="${view}" var="singleView">
<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8">
		<title>${singleView.viewTitle}</title>
...
<script type="text/javascript">
	...
	function handlePaginationClick(new_page_index, pagination_container) {
		...
		$("#pager").attr("action", "<%=request.getContextPath()%>/sxw/showSingleViewServlet?id=${singleView.id}&type=view&pageNum=" + (new_page_index + 1));
		$("#pager").submit();
		return false;
	}
...
<body>
	<!--  <div class="detail-body">
		<img src="<%=path%>${singleView.imageUrl}" alt="" class="detail-body-img">
	</div>
	-->
	...
	<div class="detail-container">
		<div class="detail-main">
			<div class="detail-mcontain">
				<h1 class="detail-title">${singleView.viewTitle}</h1>
				<time class="detail-time">${fn:substring(singleView.addtime,0,10)}</time>
				<div class="detail-writing">${singleView.content}</div>
			</div>
			<div class="detail-share">
				...
				<div class="detail-all">
					<span class="detail-lspan">浏览<b id="browsingTimes">${singleView.browsingTimes}</b>次
					</span>
						<span class="detail-rspan"> 
						<img src="../img/praise.png" alt="" class="detail-rspan-img" onclick="likes()" id="praise"><b id="praiseTimes">${singleView.likes}次</b></span>
						<span class="detail-rspan">
						<a href="#comment"><img src="../img/message1.png" alt="" class="detail-rspan-img" /></a></span>
				</div>
				...
				<ul class="comment-all">
                     ...
                     <div class="detail-discussion">
                     	 <form action="<%=path%>/sxw/addComment" method="post" id="add">
	                     	 	 ...
	                     	 	 <input name="commentId" id="commentId" style="display:none;" value="${singleView.id}"/>
	                     	 	 <input name="commentType" id="commentType" style="display:none;" value="view"/>
		...
		<div class="detail-right">
	        ...
	        <c:forEach items="${randomExperience}" var="randomExperience">
				<div class="detail-rcell">
					<a href="<%=path%>/sxw/showSingleActivityServlet?id=${randomExperience.id}&type=experience"> <img src="<%=path%>${randomExperience.imageUrl}" alt="" style="width:100%;height:150px;">
						<h3>体验：${randomExperience.experTitle}</h3>
					</a>
				</div>
	        </c:forEach>
	        <c:forEach items="${randomXiangJu}" var="randomXiangJu">
	            <c:set value="${fn:split(randomXiangJu.imageUrl,';')}" var="splitImageUrl"></c:set>
					<div class="detail-rcell">
						<c:forEach items="${splitImageUrl}" begin="0" end="0" var="outSplitImageUrl">
					    	<a href="<%=path%>/sxw/showLive"> <img src="<%=path%>${outSplitImageUrl}" alt="" style="width:100%;height:150px;"></a>
					    </c:forEach>
						<h3><a href="<%=path%>/sxw/showLive" style="color:#666;">乡居：${randomXiangJu.roomTitle}</a></h3>
				</div>
	        ...
	        <c:forEach items="${randomStory}" var="randomStory">
				<div class="detail-rcell">
					<a href="<%=path%>/sxw/showSingleStoryServlet?id=${randomStory.id}&type=story"> <img src="<%=path%>${randomStory.imageUrl}" alt="" style="width:100%;height:150px;">
...
*/
	...
	window.onload = function(){
		initAJAX();
      	var browsingTimes = ${singleView.browsingTimes} + 1;
      	var id = ${singleView.id};
	...
	initAJAX();
	xmlHttp.open("POST","<%=path%>/sxw/likes?viewStory_id=${singleView.id}&likes=findlikes",true);
	xmlHttp.send();
	xmlHttp.onreadystatechange = function(){
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200){
		var res = xmlHttp.responseText;
	...
	//alert(praiseTimes.innerHTML);
		initAJAX();
		xmlHttp.open("POST","<%=path%>/sxw/likes?viewStory_id=${singleView.id}&likes=likes",true);
		xmlHttp.send();
		xmlHttp.onreadystatechange = function(){
				if(xmlHttp.readyState == 4 && xmlHttp.status == 200){
					var res = xmlHttp.responseText;
